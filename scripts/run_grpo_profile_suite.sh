...
mkdir -p runs/grpo_profile_suite runs/grpo_profile_ablation_seed42
...
for seed in 42 43 44; do
  livebridge grpo-train \
    ...
    --output-dir "runs/grpo_profile_suite/seed${seed}" \
    --epochs 30 --seed "${seed}" --candidate-pool 50 --slate-size 10 \
    --group-size 8 --learning-rate 0.01 --residual-scale 0.35 \
    --source-weight 0.10 --profile-weight 0.05 --longtail-weight 0.05 \
    | tee "runs/grpo_profile_suite/seed${seed}.log"
done
...
for variant in no_source no_longtail; do
  ...
  livebridge grpo-train \
    ...
    --author-profile data/KuaiLive-M3/author_profile.csv \
    --output-dir "runs/grpo_profile_ablation_seed42/${variant}" \
    --epochs 30 --seed 42 --candidate-pool 50 --slate-size 10 \
    --group-size 8 --learning-rate 0.01 --residual-scale 0.35 \
    --source-weight "${source_weight}" --profile-weight 0.05 \
    --longtail-weight "${longtail_weight}" \
    | tee "runs/grpo_profile_ablation_seed42/${variant}.log"
done
